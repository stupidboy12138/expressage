...
Order.completeOrder = function(){
    ...
    location.href='http://localhost:8888/order/exportFinished';
    var ajax = new $ax(Feng.ctxPath + "/order/exportFinished", function (data) {
        Feng.success("导出订单成功！");
        // Order.table.refresh();
    }, function (data) {
        Feng.error("导出订单订单失败!" + data.responseJSON.message + "!");
    });
    // ajax.set("orderId",this.seItem.id);
    ajax.start();
};
...
$(function () {
    ...
    var table = new BSTable(Order.id, "/order/finishedList", defaultColunms);
