...
Order.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
            ...
            {title: '用户', field: 'customerName', visible: true, align: 'center', valign: 'middle'},
            {title: '收件人', field: 'recipients', visible: true, align: 'center', valign: 'middle'},
            {title: '手机号码', field: 'customerTelephone', visible: true, align: 'center', valign: 'middle'},
            {title: '收货地址', field: 'customerPlace', visible: true, align: 'center', valign: 'middle'},
            {title: '快递公司', field: 'expressageCompany', visible: true, align: 'center', valign: 'middle'},
...
Order.completeOrder = function(){
    console.log("执行到complete");
    var ajax = new $ax(Feng.ctxPath + "/order/completeOrder", function (data) {
...
Order.completeExcel = function(){
    console.log("执行到complete");
    location.href='http://localhost:8888/order/exportUnFinishedToworrow';
    var ajax = new $ax(Feng.ctxPath + "/order/exportUnFinishedToworrow", function (data) {
        Feng.success("一键完成订单成功！");
        // Order.table.refresh();
    }, function (data) {
        Feng.error("一键完成订单失败!" + data.responseJSON.message + "!");
 ...
 */
Order.completeOrder = function(){
    var ajax = new $ax(Feng.ctxPath + "/order/completeOrderTomorrow", function (data) {
        Feng.success("一键完成订单成功！");
        // Order.table.refresh();
    }, function (data) {
        Feng.error("一键完成订单失败!" + data.responseJSON.message + "!");
...
$(function () {
    ...
    var table = new BSTable(Order.id, "/order/waitingList", defaultColunms);
    table.setPaginationType("client");
    Order.table = table.init();
    // Order.table = table.bootstrapTable('destroy');
    // $("#OrderTable4").bootstrapTable('destroy');
});
